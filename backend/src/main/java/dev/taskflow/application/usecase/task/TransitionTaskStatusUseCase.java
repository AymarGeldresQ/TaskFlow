...
@Service
public class TransitionTaskStatusUseCase {
    ...
    public TransitionTaskStatusUseCase(TaskRepository taskRepository,
    ...
    @Transactional
    public TaskResponse execute(UUID taskId, TransitionTaskRequest request, UUID actorId) {
        ...
        membershipRepository.findByTeamIdAndUserId(project.getTeamId(), actorId)
            ...
            .orElseThrow(() -> new dev.taskflow.domain.exception.UnauthorizedOperationException("transition task"));
        task.transitionTo(request.status(), actorId);
